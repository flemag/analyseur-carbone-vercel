...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Message d'erreur plus spécifique pour les redirections ou blocages
        const message = error.message.includes('redirect count exceeded')
            ...
            : `Impossible d'accéder au site ${url}. Il est peut-être inaccessible, en maintenance, ou bloque les requêtes automatiques.`;
        return res.status(500).json({ message });
    }

    // *** LA CORRECTION CRUCIALE EST ICI ***
    // On vérifie si le HTML est valide AVANT de l'utiliser
    if (!html || typeof html !== 'string' || html.trim().length === 0) {
        console.error(`Le site ${url} a renvoyé une page vide.`);
        return res.status(500).json({ message: `Le site ${url} a renvoyé une page vide ou invalide.` });
    }

    // Maintenant, c'est sûr d'utiliser Cheerio
